...
public class CachedChunkPacket {
    ...
    private static byte[] computePacketBigHash(ClientboundLevelChunkWithLightPacket packet) {
        ...
        for (ClientboundLevelChunkPacketData.BlockEntityInfo blockEntitiesData : copy) {
            ...
            var registryName = BlockEntityType.getKey(blockEntitiesData.type);
            if (registryName != null) {
                digest.update(registryName.toString().getBytes(StandardCharsets.UTF_8));
            } else {
                digest.update("UNKNOWN_TYPE".getBytes(StandardCharsets.UTF_8));
            }
