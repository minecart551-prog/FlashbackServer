...
public class SimpleVoiceChatPlayback {
    ...
    public static void play(FlashbackVoiceChatSound sound) {
        try {
            VoicechatClientApi clientApi = SimpleVoiceChatPlugin.getClientApi();
            if (clientApi == null) {
                Flashback.LOGGER.warn("VoicechatClientApi not available");
                return;
            }
            ...
            if (sound instanceof FlashbackVoiceChatSound.SoundStatic) {
                ...
                Position position = clientApi.createPosition(pos.x, pos.y, pos.z);
                ClientLocationalAudioChannel channel = locationAudioChannelCache.get(source,
                        () -> clientApi.createLocationalAudioChannel(source, position));
                ...
                ClientEntityAudioChannel channel = entityAudioChannelCache.get(source,
                        () -> clientApi.createEntityAudioChannel(source));
